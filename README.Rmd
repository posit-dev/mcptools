...
rmcp can be hooked up to any application that supports MCP. To do so, use `mcp_config()` to generate the config for your machine:
...
Then, paste that output into the relevant configuration file for your application. For example:

* Claude Code: `~/.config/anthropic/mcp-config.json`
* Claude for Desktop (on macOS): `~/Library/Application Support/Claude/claude_desktop_config.json`
