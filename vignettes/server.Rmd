...
For MCP-specific metadata that does not belong to the general ellmer tool definition, wrap the tool with `mcp_server_tool()`. For example, MCP protocol versions 2025-06-18 and later support an `outputSchema` describing structured tool results:

```r
evaluate_model <- mcptools::mcp_server_tool(
  ellmer::tool(
...
mcptools::mcp_server(tools = list(evaluate_model))
```

The wrapper affects only the MCP description advertised to clients; tool execution still uses the underlying `ellmer::ToolDef`. `outputSchema` is omitted automatically when an older MCP protocol version is negotiated.
