...
logcat <- function(x, ..., append = TRUE) {
  cat(x, "\n", sep = "", append = append, file = logfile)
}
...
run_proxy <- function() {
  ...
  while (TRUE) {
    ...
    if (identical(data$method, "initialize")) {
      ...
      # No prompts yet
      cat_json(jsonrpc_response(data$id, list(prompts = list())))

    } else if (identical(data$method, "resources/list")) {
      # No resources yet
      cat_json(jsonrpc_response(data$id, list(resources = list())))
...
post_request <- function(json_data, url) {
  h <- new_handle() |>
    ...
    handle_setopt(postfields = json_data)

  result <- curl_fetch_memory(url, h)
...
jsonrpc_response <- function(id, x) {
  list(
    jsonrpc = "2.0",
    id = id,
    result = x
...
get_all_btw_tools <- function() {
  ...
  btw_tools <- unname(btw:::.btw_tools)
