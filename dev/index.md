...
Project'.](reference/figures/r_as_a_server.png)
...
components.](reference/figures/r_as_a_client.png)
...
call
[`mcptools::mcp_session()`](https://posit-dev.github.io/mcptools/dev/reference/server.md)
in those sessions. (You might include a call to this function in your
.Rprofile, perhaps using `usethis::edit_r_profile()`, to automatically
register every session you start up.)
...
will return a list of ellmer tools which you can pass directly to the
`$set_tools()` method from ellmer:
...
![A screencast of a chat with Claude. After the question is asked, a
tool called 'describe data frame' is called with the \`data_frame\`
argument set to \`forested\`. The results are returned from mcptools as
json, which the model then integrates into its response: 'Based on the
data structure, I can see there's a \`year\` column with values ranfing
from 1995 to 2024. The earliest recorded sample in the \`forested\` data
is from
1995.'](https://github.com/user-attachments/assets/064df664-2669-498e-a644-fb191a30235f)
